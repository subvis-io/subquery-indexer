import { RewardAndSlashSum } from '../types/models/RewardAndSlashSum';
import { SubstrateEvent } from '@subql/types';
import { Balance } from '@polkadot/types/interfaces';
import { NoBondRecordAccount } from '../types/models/NoBondRecordAccount';

function createSumReward(accountId: string): RewardAndSlashSum {
  const entity = new RewardAndSlashSum(accountId);
...
export async function handleBond(event: SubstrateEvent): Promise<void> {
  ...
  const entity = await RewardAndSlashSum.get(account.toString());
  if (entity === undefined) {
...
export async function handleReward(event: SubstrateEvent): Promise<void> {
  ...
  let entity = await RewardAndSlashSum.get(account.toString());
  if (entity === undefined) {
